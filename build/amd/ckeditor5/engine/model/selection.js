define(['exports', './position.js', './range.js', './liverange.js', '../../utils/emittermixin.js', './characterproxy.js', '../../utils/ckeditorerror.js', '../../utils/tomap.js', '../../utils/mix.js'], function (exports, _position, _range, _liverange, _emittermixin, _characterproxy, _ckeditorerror, _tomap, _mix) {
	/**
  ...
  */
		...
		*getRanges() {
			if (this._ranges.length) {
				for (let range of this._ranges) {
					yield _range2.default.createFromRange(range);
				}
			} else {
   ...
   */
		getFirstRange() {
			...
			return first ? _range2.default.createFromRange(first) : this._getDefaultRange();
		}
   ...
   */
		getFirstPosition() {
			return _position2.default.createFromPosition(this.getFirstRange().start);
		}
   ...
   */
		collapse(nodeOrPosition, offset) {
			const pos = _position2.default.createAt(nodeOrPosition, offset);
			const range = new _range2.default(pos, pos);
   ...
   */
		setFocus(nodeOrPosition, offset) {
			const newFocus = _position2.default.createAt(nodeOrPosition, offset);
   ...
   */
      ...
      */
					throw new _ckeditorerror2.default('selection-range-intersects: Trying to add a range that intersects with another range from selection.', { addedRange: range, intersectingRange: this._ranges[i] });
				}
			}

			this._ranges.push(_liverange2.default.createFromRange(range));
		}
   ...
   */
		_getDefaultRange() {
			...
			for (let position of _range2.default.createFromElement(defaultRoot).getPositions()) {
				if (this._document.schema.check({ name: '$text', inside: position })) {
					return new _range2.default(position, position);
				}
			}

			const position = new _position2.default(defaultRoot, [0]);
