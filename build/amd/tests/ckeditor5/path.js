define('tests', ['/ckeditor5/path.js'], function (_path) {
	/**
  ...
  */
	...
	describe('basePath', () => {
		...
		it('should work with the CKEDITOR_BASEPATH global', () => {
			...
			expect(_path2.default._getBasePath()).to.equal('http://foo.com/ckeditor/');
		});
		...
		function testGetBasePathFromTag(url, expectedBasePath) {
			it('should work with script tags - ' + url, () => {
				...
				if (typeof expectedBasePath == 'string') {
					expect(_path2.default._getBasePath()).to.equal(expectedBasePath);
				} else {
					expect(_path2.default._getBasePath()).to.match(expectedBasePath);
				}
			});
		}
	});
