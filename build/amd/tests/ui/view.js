define('tests', ['/tests/ckeditor5/_utils/utils.js', '/ckeditor5/ui/view.js', '/ckeditor5/ui/region.js', '/ckeditor5/utils/ckeditorerror.js', '/ckeditor5/ui/model.js'], function (_utils, _view, _region, _ckeditorerror, _model) {
	/**
  ...
  */
	...
	describe('View', () => {
		describe('constructor', () => {
			...
			it('accepts the model', () => {
				...
				expect(view.model).to.be.an.instanceof(_model2.default);
			...
			it('defines the locale property and the t function', () => {
				...
				view = new _view2.default(model, locale);
		...
		describe('init', () => {
			...
			it('calls child regions #init', () => {
				...
				const region1 = new _region2.default('x');
				const region2 = new _region2.default('y');

				view.register(region1, el => el);
				view.register(region2, el => el);

				const spy1 = _utils2.default.sinon.spy(region1, 'init');
				const spy2 = _utils2.default.sinon.spy(region2, 'init');
		...
		describe('register', () => {
			...
			it('should throw when first argument is neither Region instance nor string', () => {
				...
				}).to.throw(_ckeditorerror2.default, /ui-view-register-wrongtype/);
			});

			it('should throw when missing the selector argument', () => {
				expect(() => {
					view.register('x');
				}).to.throw(_ckeditorerror2.default, /ui-view-register-badselector/);
			});
			...
			it('should throw when overriding an existing region but without override flag set', () => {
				expect(() => {
					...
					view.register(new _region2.default('x'), true);
				}).to.throw(_ckeditorerror2.default, /ui-view-register-override/);
			});

			it('should register a new region with region name as a first argument', () => {
				view.register('x', true);

				expect(view.regions.get('x')).to.be.an.instanceof(_region2.default);
			});

			it('should register a new region with Region instance as a first argument', () => {
				view.register(new _region2.default('y'), true);

				expect(view.regions.get('y')).to.be.an.instanceof(_region2.default);
			});
			...
			it('should override an existing region with override flag', () => {
				...
				const region1 = new _region2.default('x');
				const region2 = new _region2.default('x');
			...
			it('should not override an existing region with the same region with override flag', () => {
				const region = new _region2.default('x');
				const spy = _utils2.default.sinon.spy(view.regions, 'remove');
		...
		describe('element', () => {
			...
			it('can be explicitly declared', () => {
				class CustomView extends _view2.default {
					constructor() {
						super();
			...
			it('is null when there is no template', () => {
				expect(new _view2.default().element).to.be.null;
			});
		});
				...
				it('allows binding attribute to the model – array of bindings (HTMLElement attribute)', () => {
					setTestViewClass(function () {
						...
						return {
							tag: 'p',
							attributes: {
								'class': ['ck-class', bind.to('foo'), bind.to('bar'), bind.to('foo', value => `foo-is-${ value }`), 'ck-end']
							},
							children: ['abc']
						};
					});
				...
				it('allows binding attribute to the model – array of bindings (Text Node)', () => {
					setTestViewClass(function () {
						...
						return {
							...
							attributes: {},
							children: [{
								text: ['ck-class', bind.to('foo'), bind.to('bar'), bind.to('foo', value => `foo-is-${ value }`), 'ck-end']
							}]
						};
					});
		...
		describe('on', () => {
			it('accepts plain binding', () => {
				...
				sinon.assert.calledWithExactly(spy, sinon.match.has('name', 'a'), sinon.match.has('target', view.element));
			});
			...
			it('accepts an array of event bindings', () => {
				...
				dispatchEvent(view.element, 'x');
				sinon.assert.calledWithExactly(spy1, sinon.match.has('name', 'a'), sinon.match.has('target', view.element));
				sinon.assert.calledWithExactly(spy2, sinon.match.has('name', 'b'), sinon.match.has('target', view.element));
			});
			...
			it('accepts DOM selectors', () => {
				...
				expect(spy1.firstCall.calledWithExactly(sinon.match.has('name', 'a'), sinon.match.has('target', view.element.firstChild))).to.be.true;
				...
				expect(spy2.firstCall.calledWithExactly(sinon.match.has('name', 'b'), sinon.match.has('target', view.element.lastChild))).to.be.true;
				...
				expect(spy1.secondCall.calledWithExactly(sinon.match.has('name', 'a'), sinon.match.has('target', view.element.lastChild.firstChild))).to.be.true;
			...
			it('accepts function callbacks', () => {
				...
				sinon.assert.calledWithExactly(spy1, sinon.match.has('target', view.element));

				sinon.assert.calledWithExactly(spy2, sinon.match.has('target', view.element.firstChild));
			});
			...
			it('supports event delegation', () => {
				...
				sinon.assert.calledWithExactly(spy, sinon.match.has('name', 'a'), sinon.match.has('target', view.element.firstChild));
			});
		...
		describe('destroy', () => {
			...
			it('destroys child regions', () => {
				const region = new _region2.default('x');
				const spy = _utils2.default.sinon.spy(region, 'destroy');
				const regionsRef = view.regions;
				const regionViewsRef = region.views;

				view.register(region, true);
				view.regions.get('x').views.add(new _view2.default());
				view.destroy();
		...
		describe('applyTemplateToElement', () => {
			...
			it('should work for deep DOM structure', () => {
				...
				expect(el.outerHTML).to.equal('<div id="FOO" class="applied-parent-42">' + '<a class="applied-A-42" id="applied-A">Text applied to childA.</a>' + '<b class="applied-B-42" id="applied-B">Text applied to childB.</b>' + '</div>');

				view.model.b = 16;

				expect(el.outerHTML).to.equal('<div id="FOO" class="applied-parent-16">' + '<a class="applied-A-16" id="applied-A">Text applied to childA.</a>' + '<b class="applied-B-16" id="applied-B">Text applied to childB.</b>' + '</div>');
			...
			it('shares a template definition between View instances – event listeners', () => {
				const el = document.createElement('div');
				const spy = _utils2.default.sinon.spy();
				const view1 = new _view2.default();
				const view2 = new _view2.default();
				const def = {
					tag: 'div',
					on: {
						click: spy
	...
	function setTestViewClass(templateFn, regionsFn) {
		TestView = class V extends _view2.default {
			constructor(model) {
				super(model);
	...
	function setTestViewInstance(model) {
		view = new TestView(new _model2.default(model));
