define('tests', ['/ckeditor5/utils/config.js'], function (_config) {
	/**
  ...
  */
	...
	describe('constructor', () => {
		it('should set configurations', () => {
			...
			expect(config).to.have.property('resize').to.have.property('icon').to.have.property('path').to.equal('xyz');
	...
	describe('set', () => {
		it('should create Config instances for objects', () => {
			expect(config.resize).to.be.an.instanceof(_config2.default);
			expect(config.resize.icon).to.be.an.instanceof(_config2.default);
		});
		...
		it('should set configurations when passing objects', () => {
			...
			expect(config).to.have.property('option1').to.equal(1);

			expect(config).to.have.property('option2').to.have.property('suboption21').to.equal(21);
		});
		...
		it('should set configurations when passing name.with.deep and value', () => {
			...
			expect(config).to.have.property('color').to.have.property('red').to.equal('f00');

			expect(config).to.have.property('background').to.have.property('color').to.have.property('blue').to.equal('00f');
		});
		...
		it('should replace a simple entry with a Config instance', () => {
			...
			expect(config.test).to.be.an.instanceof(_config2.default);
		...
		it('should replace a simple entry with a Config instance when passing a name.with.deep', () => {
			...
			expect(config.test).to.be.an.instanceof(_config2.default);
			expect(config.test.prop).to.be.an.instanceof(_config2.default);
